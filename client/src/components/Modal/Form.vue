<template>
  <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-200 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0" >
    <div v-if="formStatus" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" >
      <div class="flex min-h-screen items-end justify-center px-6 pt-6 pb-6 text-center sm:block sm:p-0">
        ...
        <div class="relative inline-block rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8   sm:align-middle px-6 py-6" :class="width">
          ...
          <div class="max-h-[65vh] overflow-y-auto py-6 px-3">
            <slot></slot>
          </div>
          <div class="flex justify-end gap-4 pt-6">
            ...
            <PrimaryButton type="submit" @click="handleSubmit"><IconPlus></IconPlus>{{submitLabel}}</PrimaryButton>
          </div>
        </div>
      </div>
    </div>
