  ...
  const action = async () => {
    try {
      if (isEdit) {
        ...
        emit('status', { error: false, err_msg: (response?.data?.err_msg || response?.data?.message) || 'Bank berhasil di update' });
      } else {
        const response = await addBank(Bank.value);
        showConfirmDialog.value = false;
        emit('status', { error: false, err_msg: (response?.data?.err_msg || response?.data?.message) || 'Bank berhasil di tambahkan' });
      }
      emit('close')
    } catch (error) {
      showConfirmDialog.value = false;
