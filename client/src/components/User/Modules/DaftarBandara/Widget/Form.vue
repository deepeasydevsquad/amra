...
import { addBandara, editBandara } from '@/service/daftar_bandara';
...
const Bandara = ref<Partial<Bandara>>({
  name: '',
});
...
const saveData = async () => {
  ...
  const isEdit = !!Bandara.value.id;

  const action = async () => {
    try {
      if (isEdit) {
        const response = await editBandara(Bandara.value.id, Bandara.value );
        showConfirmDialog.value = false;
        emit('status', { error: false, err_msg: response.error_msg || 'Bandara berhasil di update' });
      } else {
        const response = await addBandara(Bandara.value);
        showConfirmDialog.value = false;
        emit('status', { error: false, err_msg: response.error_msg || 'Bandara berhasil di tambahkan' });
      }
      emit('close')
    } catch (error) {
      showConfirmDialog.value = false;
...
onMounted(() => {
  Bandara.value = { ...Bandara.value, ...(props.selectedBandara ?? {}) };
})
      ...
      <div class="relative inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <h3 class="text-2xl flex justify-center font-bold leading-6 text-gray-900 mb-4">
            {{ Bandara.id ? "Edit Data Bandara" : "Tambah Bandara Baru" }}
          </h3>
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Nama</label>
              <input
                v-model="Bandara.name"
                type="text"
        ...
        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
          <PrimaryButton
            @click="saveData"
          >
            {{ Bandara.id ? "Simpan Perubahan" : "Tambah" }}
          </PrimaryButton>
          <button
            @click="emit('close')"
