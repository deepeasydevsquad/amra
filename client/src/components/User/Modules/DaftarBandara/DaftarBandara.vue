...
const validateForm = (): boolean => {
  ...
  if (!selectedBandara.value.name?.trim()) {
...
const saveData = async () => {
  ...
  const isEdit = !!selectedBandara.value.id;
  const action = async () => {
    try {
      if (isEdit) {
        const response = await editBandara(selectedBandara.value.id, selectedBandara.value );
        showConfirmDialog.value = false;
        displayNotification(response.error_msg);
      } else {
        const response = await addBandara(selectedBandara.value);
        showConfirmDialog.value = false;
        displayNotification(response.error_msg);
      }
          ...
          <div class="relative inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              <h3 class="text-2xl flex justify-center font-bold leading-6 text-gray-900 mb-4">
                {{ selectedBandara.id ? "Edit Data Bandara" : "Tambah Bandara Baru" }}
              </h3>
              <div class="space-y-4">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-1">Nama</label>
                  <input
                    v-model="selectedBandara.name"
                    type="text"
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-600 font-normal" placeholder="Nama Bandara"
                  />
            ...
            <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
              ...
              >
                {{ selectedBandara.id ? "Simpan Perubahan" : "Tambah" }}
              </button>
              <button
