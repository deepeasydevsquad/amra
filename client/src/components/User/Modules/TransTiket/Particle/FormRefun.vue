...
const validateForm = (): boolean => {
  ...
  if( form.value.sudah_bayar > ( form.value.refund  + form.value.fee)) {
    console.log("----ZZZZ----5");
    errors.value.refund = 'Nominal refund plus Fee tidak boleh lebih besar dari yang sudah dibayarkan'
    isValid = false
  }
...
<template>
    <Form :formStatus="props.formStatus" :label="'Form Refund Tiket'" :width="'w-1/4'" :submitLabel="'REFUND'" @submit="handleSubmit" @cancel="handleCancle">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-0 pr-3">
        ...
        <div class="mt-0 md:col-span-2">
          <label for="kode_booking" class="text-sm font-medium text-gray-700 mb-1">Kode Booking</label>
          <div id="kode_booking" class="bg-gray-100 text-gray-800 px-3 py-2 mt-1 rounded border border-gray-300">
            {{ form.kode_booking }}
          </div>
        </div>
        <div class="md:col-span-4 mt-0 md:col-span-1">
          <label for="maskapai" class="text-sm font-medium text-gray-700 mb-1">Maskapai</label>
          <div id="maskapai" class="bg-gray-100 text-gray-800 px-3 py-2 mt-1 rounded border border-gray-300">
            {{ form.maskapai }}
          </div>
        </div>
        <div class="md:col-span-1 mt-0 md:col-span-1">
          <label for="pax" class="text-sm font-medium text-gray-700 mb-1">Pax</label>
          <div id="pax" class="bg-gray-100 text-gray-800 px-3 py-2 mt-1 rounded border border-gray-300">
            {{ form.pax }}
          </div>
        </div>
        <div class="md:col-span-3 mt-0 md:col-span-1">
          <label for="harga" class="text-sm font-medium text-gray-700 mb-1">Harga</label>
          <div id="harga" class="bg-gray-100 text-gray-800 px-3 py-2 mt-1 rounded border border-gray-300">
        ...
        <InputCurrency class="mt-0 md:col-span-2" label="Nominal Refund" id="refund" placeholder="Nominal Refund" v-model="form.refund" :error="errors.refund"/>
        <InputCurrency class="mt-0 md:col-span-2" label="Nominal Fee" id="fee" placeholder="Nominal fee" v-model="form.fee" :error="errors.fee"/>
      </div>
    </Form>
