...
<template>
  <div class="py-4 px-0 bg-white rounded-b-lg  text-gray-800">
    ...
    <div v-else>
      <vueper-slides class="no-shadow w-full" :visible-slides="4.5" slide-multiple :arrows="false" :bullets="false" :gap="2" :slide-ratio="1 / 6"
        :dragging-distance="70" :breakpoints="{ 800: { visibleSlides: 2, slideMultiple: 2 } }" fixed-height="750px" >
        <vueper-slide v-if="daftarPaket.length" v-for="paket in daftarPaket" :key="paket.id" class="flex items-center justify-center" content-visible fixed-height="750px">
          <template #content>
            <div class="bg-white border rounded-lg shadow-sm hover:shadow-md transition" >
              ...
              <div class="pr-4 pl-4 space-y-8 pb-8 pt-4">
                <div class="grid grid-cols-2 gap-y-2 text-sm px-0">
                    ...
                    <div class="flex items-center gap-1">
                      <font-awesome-icon :icon="['fas', 'qrcode']" class="w-4 h-4 text-gray-600 pr-2" /><span>Kode Paket</span>
                    </div>
                    <div class="ml-6 font-semibold text-right">{{ paket.kode }}</div>
                    <div class="flex items-center gap-1">
                      <font-awesome-icon :icon="['fas', 'calendar']" class="w-4 h-4 text-gray-600 pr-2" /><span>Jdwl. Berangkat</span>
                    </div>
                    <div class="ml-6 font-semibold text-right">{{ paket.departure_date }}</div>
                    <div class="flex items-center gap-1">
                      <font-awesome-icon :icon="['far', 'clock']" class="w-4 h-4 text-gray-600 pr-2" /><span>Durasi Perjalanan</span>
                    </div>
                    <div class="ml-6 font-semibold text-right">{{ paket.durasi }} Hari</div>
                    <div class="flex items-center gap-1">
                      <font-awesome-icon :icon="['far', 'user']" class="w-4 h-4 text-gray-600 pr-2" /><span>Total Jamaah</span>
                    </div>
                    <div class="ml-6 font-semibold text-right">{{ paket.total_jamaah }} Orang</div>
                    <div class="flex items-start gap-1">
                      <font-awesome-icon :icon="['fas', 'money-bill']" class="w-4 h-4 text-gray-600 pr-2" /><span>Harga</span>
                    </div>
                    <div class="flex flex-col justify-start min-h-[2.5rem]">
                      ...
                      <span v-if="paket.prices.min !== paket.prices.max" class="ml-6 font-semibold text-blue-600 text-right" >
