    ...
    <div class="w-full flex justify-end mb-4">
      <div class="inline-flex rounded-md shadow-xs" role="group">
        ...
        <input type="text" id="search" class="block w-64 px-3 py-2 text-gray-700 bg-white border border-gray-300 rounded-s-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200"
          v-model="searchQuery" @change="fetch_data()" placeholder="Cari data..." />
        <select v-model="selectedOptionCabang" style="width: 300px" @change="fetch_data()"
  ...
  <div class="overflow-hidden rounded-lg border border-gray-200 shadow-md">
    <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
      <thead class="bg-gray-100">
        <tr>
          <th class="text-center font-medium px-6 text-gray-900 py-3 text-sm w-[35%]">Info Member</th>
          <th class="text-center font-medium px-6 text-gray-900 py-3 text-sm w-[15%]">No WhatsApp</th>
      ...
      <tbody>
        <tr v-for="(item, index) in data" :key="index" class="border-b border-gray-100 hover:bg-gray-50" >
            ...
            <table class="w-full mb-5 border mt-3">
              <tbody>
                <tr class="border-gray-200 hover:bg-gray-200">
                  ...
                  <td class="border-b text-right space-y-2 text-sm px-6 py-2">{{ item.member_fullname }}</td>
                </tr>
                <tr class="border-gray-200 hover:bg-gray-200">
                  <td class="border-b px-6 py-2">JENIS IDENTITAS</td>
                  <td class="text-center border-b py-2">:</td>
                  <td class="border-b text-right space-y-2 text-sm px-6 py-2">{{ item.member_identity_type == 'ktp' ? 'KTP' : 'PASSPORT'}}</td>
                </tr>
                <tr class="border-gray-200 hover:bg-gray-200">
                  <td class="border-b px-6 py-2">NOMOR IDENTITAS</td>
                  <td class="text-center border-b py-2">:</td>
                  <td class="border-b text-right space-y-2 text-sm px-6 py-2">{{ item.member_identity_number }}</td>
                </tr>
                <tr class="border-gray-200 hover:bg-gray-200">
                  <td class="border-b px-6 py-2">JENIS KELAMIN</td>
                  <td class="text-center border-b py-2">:</td>
                  <td class="border-b text-right space-y-2 text-sm px-6 py-2">{{ item.member_gender == 'laki_laki' ? 'Laki-laki' : 'Perempuan' }}</td>
                </tr>
            ...
            <table class="w-full mb-5 border mt-3">
              <tbody>
                <tr class="border-gray-200 hover:bg-gray-200">
                  ...
                  <td class="text-center border-b py-2">:</td>
                  <td class="border-b text-right space-y-2 text-sm px-6 py-2">{{ item.member_level_keagenan }}</td>
                </tr>
                <tr class="border-gray-200 hover:bg-gray-200">
                  <td class="border-b px-6 py-2">FEE SUDAH DIBAYAR</td>
                  <td class="text-center border-b py-2">:</td>
                  <td class="border-b text-right space-y-2 text-sm px-6 py-2">{{ formatRupiah(item.total_fee_lunas) }}</td>
                </tr>
                <tr class="border-gray-200 hover:bg-gray-200">
                  <td class="border-b px-6 py-2">FEE BELUM DIBAYAR</td>
                  <td class="text-center border-b py-2">:</td>
                  <td class="border-b text-right space-y-2 text-sm px-6 py-2">{{ formatRupiah(item.total_fee_belum_lunas) }}</td>
                </tr>
                <tr class="border-gray-200 hover:bg-gray-200">
                  <td class="border-b px-6 py-2">TOTAL TRANSAKSI</td>
                  <td class="text-center border-b py-2">:</td>
                  <td class="border-b text-right space-y-2 text-sm px-6 py-2">{{ item.jumlah_transaksi }} Transaksi</td>
                </tr>
              </tbody>
            </table>
          </td>
