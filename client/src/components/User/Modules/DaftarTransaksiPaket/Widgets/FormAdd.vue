...
import { getJamaah, getPaketTypes, getAgen, addTransaksiPaket } from "@/service/daftar_transaksi_paket"
...
interface PaketTypes { id: number; name: string, price: number }
  ...
  showConfirmation(
    ...
    async () => {
      ...
      try {
        ...
        emit('status', { error: true, err_msg: error.response.data.error_msg || 'Terjadi kesalahan saat menyimpan data.' })
      } finally {
        isLoading.value = false
        isSaving = false
      }
