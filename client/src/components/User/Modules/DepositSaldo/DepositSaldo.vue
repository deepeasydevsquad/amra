...
<template>
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200 shadow-md">
      <table class="w-full border-collapse bg-white text-left text-sm text-gray-500">
        ...
        <tbody class="divide-y divide-gray-100 border-t border-gray-100">
          ...
          <!-- Deposit Data -->
          <tr v-for="deposit in deposits" :key="deposit.id" class="hover:bg-gray-50 transition-colors" >
              ...
              <table class="w-full">
                <tbody>
                  <tr>
                    ...
                    <td>:</td>
                    <td class="text-right space-y-2 text-sm py-1">{{ deposit.nominal ? 'Rp' + new Intl.NumberFormat('id-ID').format(deposit.nominal) : '-' }}</td>
                  </tr>
                  <tr>
                    <td class="w-[30%]">Saldo Sebelum:</td>
                    <td>:</td>
                    <td class="text-right space-y-2 text-sm py-1">{{ deposit.saldo_sebelum ? 'Rp' + new Intl.NumberFormat('id-ID').format(deposit.saldo_sebelum) : '-' }}</td>
                  </tr>
                  <tr>
  ...
  <Confirmation :showConfirmDialog="showDeleteConfirmDialog" confirmTitle="Konfirmasi Hapus" confirmMessage="Apakah Anda yakin ingin menghapus deposit ini?">
