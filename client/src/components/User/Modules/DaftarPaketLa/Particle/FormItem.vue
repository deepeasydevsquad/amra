  ...
  <div v-if="isFormItemOpen" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center bg-gray-500 bg-opacity-75">
    <div class="bg-white rounded-lg shadow-xl w-full max-w-6xl p-6">
      ...
      <div class="overflow-x-auto bg-gray-400 rounded-lg overflow-y-auto shadow-md max-h-[60vh]">
        <table class="w-full border-collapse border border-gray-100 shadow-lg rounded-lg overflow-hidden">
          <thead>
          ...
          <tbody>
            <tr v-for="(item, index) in items" :key="index" class="bg-gray-200 text-gray-600">
              <td class="border border-gray-300 p-2">
                <textarea v-model="item.deskripsi" rows="2" class="w-full p-2 border rounded resize-y" placeholder="Deskripsi"></textarea>
                <p v-if="errors[index]?.deskripsi" class="text-sm text-red-600">{{ errors[index].deskripsi }}</p>
              </td>
              <td class="border border-gray-300 p-2">
                <input v-model="item.checkIn" type="date" class="w-full p-2 border rounded">
                <p v-if="errors[index]?.checkIn" class="text-sm text-red-600">{{ errors[index].checkIn }}</p>
              </td>
              <td class="border border-gray-300 p-2">
                <input v-model="item.checkOut" type="date" class="w-full p-2 border rounded">
                <p v-if="errors[index]?.checkOut" class="text-sm text-red-600">{{ errors[index].checkOut }}</p>
              </td>
              <td class="border border-gray-300 p-2">
                <input v-model="item.day" type="number" class="w-full p-2 border rounded" placeholder="Day">
                <p v-if="errors[index]?.day" class="text-sm text-red-600">{{ errors[index].day }}</p>
              </td>
              <td class="border border-gray-300 p-2">
                <input v-model="item.pax" type="number" class="w-full p-2 border rounded" placeholder="Pax">
          ...
          <tfoot>
            <tr class="bg-gray-300 text-gray-600">
              <td colspan="7" class="text-end p-2">
                  <button @click="addItem" class="mt-3 bg-gray-100 hover:bg-gray-200 text-gray-700 py-2 px-4 rounded">+ Tambah Row Baru</button>
              </td>
            </tr>
          </tfoot>
        </table>
      ...
      <div class="bg-gray-50 p-5 sm:flex sm:flex-row-reverse sm:px-3">
        <button @click="saveData" class="inline-flex w-full justify-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm">Lanjutkan</button>
        <button @click="$emit('close')" class="mt-3 inline-flex w-full justify-center rounded-md border border-gray-400 bg-gray-200 px-4 py-2 text-base font-medium text-gray-800 shadow-sm hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Cancel</button>
      </div>
    </div>
  </div>
