...
<template>
  <div v-if="isFormPembayaranOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex min-h-screen justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="relative inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-5xl sm:align-middle">
        <div class="p-6 mx-auto bg-white">
          <h3 class="text-2xl flex justify-center font-bold leading-6 text-gray-900 mb-4">Form Pembayaran Transaksi Paket LA</h3>
          <div class="space-y-4 text-gray-600">
            <!-- Tabel Dummy -->
            <div class="overflow-x-auto rounded-lg overflow-y-auto scroll-smooth shadow-md max-h-[33vh] border mb-4">
              <table class="w-full mt-2 border text-center text-xs mb-3">
                ...
                <tbody>
                  ...
                  <tr v-else>
                    <td colspan="6" class="text-center text-sm p-4 text-gray-600">Daftar riwayat transaksi paket la tidak ditemukan</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              <div>
                ...
                <p class="w-full border rounded p-2 bg-gray-100">Rp {{ transaksi?.total_price.toLocaleString() }}</p>
              </div>
              <div>
