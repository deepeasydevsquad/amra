<template>
  ...
  <div class="bg-white p-6 rounded-lg w-full">
    <h2 class="text-gray-700 text-left text-2xl font-semibold mb-6">Tambah Member</h2>
    ...
    <div v-if="types === 'administrator'" class="mb-6">
      ...
      <select id="cabang" v-model="form.cabang_id" class="w-full text-gray-700 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        :class="{ 'border-red-500': errors.cabang_id }" >
        <option value="0" disabled>Pilih Cabang</option>
        <option v-for="cabang in cabangs" :key="cabang.id" :value="cabang.id">{{ cabang.name }}</option>
      </select>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
      ...
      <div>
        <label for="birthplace" class="block text-sm font-medium text-gray-700 mb-2">Tempat Lahir</label>
        <input
          type="text"
          id="birthplace"
      ...
      <div>
        <label for="birthdate" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Lahir</label>
        <input
          type="date"
          id="birthdate"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
      <div>
        <label for="bank-list" class="block text-sm font-medium text-gray-700 mb-2">Jenis Identitas</label>
        <select
          id="bank-list"
    ...
    <div class="flex justify-end gap-4">
      <button type="button" @click="handleCancel" class="px-6 py-2 text-sm font-medium text-gray-700 bg-gray-100 border border-gray-300 rounded-lg hover:bg-gray-200" >
        CANCEL
      </button>
      <PrimaryButton type="submit" @click="handleSubmit"><IconPlus></IconPlus> Tambah Member</PrimaryButton>
    </div>
  </div>
</template>
