...
import { addProviderVisa, editProviderVisa } from '@/service/daftar_provider_visa';
...
const ProviderVisa = ref<Partial<ProviderVisa>>({
  name: '',
});
...
const validateForm = (): boolean => {
  ...
  if (!ProviderVisa.value.name?.trim()) {
...
const saveData = async () => {
  ...
  const isEdit = !!ProviderVisa.value.id;
  const action = async () => {
    try {
      if (isEdit) {
        const response = await editProviderVisa(ProviderVisa.value.id, ProviderVisa.value );
        showConfirmDialog.value = false;
        emit('status', { error: false, err_msg: (response?.error_msg || response?.data?.message) || 'Provider Visa berhasil di update' });
      } else {
        const response = await addProviderVisa(ProviderVisa.value);
        showConfirmDialog.value = false;
        emit('status', { error: false, err_msg: (response?.error_msg || response?.data?.message) || 'Provider Visa berhasil di tambahkan' });
      }
      emit('close')
    } catch (error) {
      showConfirmDialog.value = false;
...
onMounted(() => {
  ProviderVisa.value = { ...ProviderVisa.value, ...(props.selectedProviderVisa ?? {}) };
})
</script>
      ...
      <div class="relative inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <h3 class="text-2xl flex justify-center font-bold leading-6 text-gray-900 mb-4">
            {{ ProviderVisa.id ? "Edit Data Provider Visa" : "Tambah Provider Visa Baru" }}
          </h3>
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Nama</label>
              <input
                v-model="ProviderVisa.name"
                type="text"
        ...
        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
          <PrimaryButton
            @click="saveData"
          >
            {{ ProviderVisa.id ? "Simpan Perubahan" : "Tambah" }}
          </PrimaryButton>
          <button
            @click="emit('close')"
