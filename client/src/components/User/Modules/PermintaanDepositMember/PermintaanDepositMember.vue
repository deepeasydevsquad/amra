    ...
    <div class="overflow-x-auto rounded-lg border border-gray-200 shadow-md">
      <table class="w-full bg-white text-left text-sm text-gray-700">
        ...
        <tbody class="divide-y divide-gray-200">
            ...
            <tr v-for="deposit in dataDepositMember" :key="deposit.id" class="hover:bg-gray-50">
              ...
              <td class="p-3 align-top">
                <div class="flex flex-col gap-1">
                  <div>
                    <span v-if="deposit.status === 'disetujui'" class="px-2 py-1 text-xs font-medium text-green-800 bg-green-100 rounded-full">Disetujui</span>
                    <span v-else-if="deposit.status === 'ditolak'" class="px-2 py-1 text-xs font-medium text-red-800 bg-red-100 rounded-full">Ditolak</span>
                    <span v-else class="px-2 py-1 text-xs font-medium text-yellow-800 bg-yellow-100 rounded-full">Diproses</span>
                  </div>
                  <div>
                    <span v-if="deposit.sending_payment_status === 'sudah_dikirim'" class="px-2 py-1 text-xs font-medium text-blue-800 bg-blue-100 rounded-full">Sudah Dikirim</span>
                    <span v-else class="px-2 py-1 text-xs font-medium text-gray-800 bg-gray-100 rounded-full">Belum Dikirim</span>
                  </div>
                  <p v-if="deposit.petugas && deposit.petugas !== '-'" class="text-xs text-gray-500 mt-1">Oleh: {{ deposit.petugas }}</p>
                </div>
              </td>
          ...
          <tr v-else>
            <td :colspan="totalColumns" class="p-4 text-center text-gray-500">Daftar Permintaan tidak ditemukan.</td>
          </tr>
        </tbody>
