...
const validateForm = (): boolean => {
  ...
  if (!form.id) {
    showConfirmation(
      'Konfirmasi',
      'ID tabungan tidak ditemukan. Silakan coba lagi.',
      () => {
        emit('close');
      }
    );
...
const cetakDataJamaah = async () => {
  ...
  showConfirmation(
    'Konfirmasi Cetak',
    'Anda yakin ingin mencetak data jamaah?',
    async () => {
      try {
        isLoading.value = true
        const url = `/daftar-jamaah-paket/cetak-data-jamaah/${props.transpaketId}/cetak?petugasId=${form.petugas_id}`;
...
<template>
  ...
  <div v-if="props.isFormCetakDataJamaahOpen && !isLoading" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex min-h-screen items-end justify-center px-6 pt-6 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="relative p-6 inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
        <div class="mb-4">
          ...
          <label for="Petugas" class="block text-sm font-medium text-gray-700">Tanda tangan petugas</label>
          <select
            id="Petugas_id"
