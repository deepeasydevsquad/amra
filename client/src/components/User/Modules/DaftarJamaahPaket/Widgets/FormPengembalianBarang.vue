...
import { getInfoPengembalianBarang, pengembalianHandoverBarang } from "@/service/daftar_jamaah_paket"
...
const fetchData = async () => {
  if (!props.transpaketId) {
    displayNotification('ID Transaksi Paket kosong', 'error')
    return
  }

  try {
    isLoading.value = true
    const response = await getInfoPengembalianBarang(props.transpaketId)
...
const validateForm = (): boolean => {
  ...
  const dikembalikanItems = dataRiwayatHandoverBarang.value.filter(item =>
    selectedItems.value.includes(item.id) && item.status.toLowerCase() === 'dikembalikan'
  )
  ...
  showConfirmation(
    ...
    async () => {
      ...
      try {
        ...
        window.open(`/kwitansi-pengembalian-handover-barang-paket/${response.data.invoice}`, '_blank')
          ...
          <div class="overflow-y-auto max-h-[64vh] no-scrollbar px-1">
            ...
            <div class="overflow-auto">
              ...
              <table class="w-full text-gray-800 text-sm border border-gray-300">
                ...
                <tbody>
                  <!-- Loop baris item -->
                  <tr v-for="(item, index) in dataRiwayatHandoverBarang" :key="index" class="border-t text-center align-top">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 text-gray-600">
              <div>
                <input v-model="form.receiver_returned" type="text" placeholder="Nama Penerima Barang" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-600 font-normal" />
                <p v-if="errors.receiver_returned" class="mt-1 text-sm text-red-600">{{ errors.receiver_returned }}</p>
              </div>
              <div>
                <input v-model="form.receiver_returned_identity" type="text" placeholder="No Identitas Penerima Barang" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-600 font-normal" />
                <p v-if="errors.receiver_returned_identity" class="mt-1 text-sm text-red-600">{{ errors.receiver_returned_identity }}</p>
              </div>
              <div>
                <input v-model="form.receiver_returned_hp" type="text" placeholder="No HP Penerima Barang" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-600 font-normal" />
                <p v-if="errors.receiver_returned_hp" class="mt-1 text-sm text-red-600">{{ errors.receiver_returned_hp }}</p>
              </div>
            </div>

            <div class="mb-4 text-gray-600">
              <textarea v-model="form.receiver_returned_address" rows="3" placeholder="Alamat Penerima Barang" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-gray-600 font-normal"></textarea>
              <p v-if="errors.receiver_returned_address" class="mt-1 text-sm text-red-600">{{ errors.receiver_returned_address }}</p>
            </div>
