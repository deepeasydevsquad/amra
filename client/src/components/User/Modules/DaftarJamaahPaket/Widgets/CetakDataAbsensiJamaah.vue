...
watch(data, () => {
  if (data.value) {
    setTimeout(() => {
      ...
      setTimeout(() => {
        if (window.opener) {
          window.opener.postMessage({ event: 'sukses', message: 'Data absensi jamaah berhasil dicetak' }, '*');
        }
        window.close();
      }, 500);
    }, 500);
...
onMounted(async () => {
  ...
  if (!data.value || data.value.length === 0) {
    if (window.opener) {
      window.opener.postMessage({ event: 'gagal', message: 'Data absensi jamaah tidak ditemukan' }, '*');
      window.close();
    } else {
      window.close();
    }
