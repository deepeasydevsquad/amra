...
const validateForm = (): boolean => {
  ...
  if (!form.id) {
    showConfirmation(
      'Konfirmasi',
      'ID paket tidak ditemukan. Silakan coba lagi.',
      () => {
        emit('close');
      }
    );
...
const cetakDataJamaah = async () => {
  ...
  showConfirmation(
    'Konfirmasi Cetak',
    'Anda yakin ingin mencetak absensi jamaah?',
    async () => {
      try {
        isLoading.value = true
...
<template>
  ...
  <div v-if="props.isFormDownloadAbsensiOpen && !isLoading" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
