...
const validateForm = (): boolean => {
  ...
  if( formData.value.kapasitas_bus != 0 &&  formData.value.kapasitas_bus < formData.value.jamaah_ids.length) {
  ...
  <Form :form-status="isFormOpen" :label="formData.id === 0 ? 'Tambah Bus' : 'Edit Bus'" @close="handleCancel" @cancel="handleCancel" @submit="handleSubmit" width="sm:w-full sm:max-w-xl" :submitLabel="formData.id === 0 ? 'TAMBAH BUS' : 'PERBAHARUI BUS'">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        ...
        <input id="bus_number" type="text" placeholder="Nomor Bus contoh: BL 12345 UA" v-model="formData.bus_number"
      ...
      <div>
        ...
        <input id="bus_leader" type="text" placeholder="Pemimpin Bus" v-model="formData.bus_leader"
          class="block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-black placeholder:text-gray-400"
        />
      </div>
      <div>
        <label for="kapasitas" class="block text-sm font-medium text-black mb-1">Kapasitas Bus</label>
        <input type="number" id="kapasitas" v-model="formData.kapasitas_bus" min="1"
      ...
      <div>
        ...
        <select id="city" v-model="formData.city_id" class="block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 text-black placeholder:text-gray-400">
      ...
      <div class="md:col-span-2">
        ...
        <button type="button" @click="addRow" class="w-full mt-2 px-4 py-2 border border-dashed border-gray-300 text-sm font-medium rounded-md text-black hover:bg-gray-50" >
          + Tambah Jamaah
        </button>
      </div>
    </div>
