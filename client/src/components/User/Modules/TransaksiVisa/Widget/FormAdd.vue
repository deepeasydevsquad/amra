...
interface DropdownOption { value: number | string; label: string; }
interface CityOption { id: number; name: string; kode: string; }
...
const isFormFilled = (): boolean => form.payer.trim() !== '' || form.payer_identity.trim() !== '' || form.passport_number.trim() !== '';
...
const displayNotification = (message: string, type: 'success' | 'warning' | 'error' = 'success') => {
...
const saveCurrentEntry = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      displayNotification(response.error_msg || `Gagal menyimpan entri ${currentEntryIndex.value + 1}.`, 'error');
    }
  } catch (error: any) {
    const errorMessage = error?.response?.data?.message || error?.response?.data?.error_msg || 'Terjadi kesalahan jaringan.';
...
const nextStep = () => { if (validateStep(currentStep.value) && currentStep.value < totalSteps) currentStep.value++; };
...
onMounted(async () => {
  ...
  if (citiesFromApi && citiesFromApi.length > 0) {
    cityOptions.value = citiesFromApi.map((city: CityOption) => ({ value: city.id, label: `${city.name} (${city.kode})` }));
  }

  const visaTypesFromApi = await getVisaTypesList();
  if (visaTypesFromApi && visaTypesFromApi.length > 0) {
    visaTypeOptions.value = visaTypesFromApi.map((visaType: any) => ({ value: visaType.name, label: visaType.name }));
  }
});
...
<template>
  <div v-if="isFormOpen" class="fixed inset-0 z-50 overflow-y-auto">
    <div class="flex min-h-screen items-center justify-center px-2 py-2">
      ...
      <div class="relative w-full max-w-2xl max-h-xl transform rounded-lg bg-white text-left shadow-xl transition-all">
        <!-- Header -->
        <div class="bg-white px-4 py-2 border-b border-gray-200 rounded-t-lg flex justify-between items-center">
            <h3 class="text-lg font-bold text-gray-800">Tambah Transaksi Visa</h3>
            <button @click="resetFormAndClose" class="text-gray-400 hover:text-gray-600 text-xl">&times;</button>
        </div>

        <!-- Entry Management -->
        <div class="px-4 py-2 bg-gray-50 border-b border-gray-200">
          <div class="flex items-center justify-between mb-2">
            <h4 class="text-xs font-semibold text-gray-700">Kelola Entries ({{ formEntries.length }})</h4>
          ...
          <div class="flex flex-wrap gap-1">
            <template v-for="(entry, index) in formEntries" :key="`entry-${index}`">
              <div class="flex items-center bg-white border rounded px-2 py-1 text-xs transition-all" :class="{ 'ring-2 ring-blue-500 shadow-md': currentEntryIndex === index }">
                
                <span v-if="entry.status === 'success'" title="Berhasil Disimpan" class="mr-2 text-green-500 text-base">✅</span>
                <span v-if="entry.status === 'duplicate'" title="Data Duplikat" class="mr-2 text-yellow-500 text-base">⚠️</span>
                <span v-if="entry.status === 'error'" title="Gagal Disimpan" class="mr-2 text-red-500 text-base">❌</span>

                <button @click="switchToEntry(index)" :class="currentEntryIndex === index ? 'text-blue-600 font-semibold' : 'text-gray-600'" class="hover:text-blue-600">
                  Entry {{ index + 1 }} {{ entry.payer ? `(${entry.payer})` : '(Kosong)' }}
                </button>
                <button v-if="formEntries.length > 1" @click="removeEntry(index)" class="ml-2 text-red-500 hover:text-red-700 font-bold">
                  ×
                </button>
              </div>
            </template>
        ...
        <div class="px-4 py-3">
            <div class="flex items-center">
                ...
                <div class="flex items-center" :class="step <= currentStep ? 'text-gray-800' : 'text-gray-400'">
                    <div class="w-6 h-6 flex items-center justify-center rounded-full border-2 font-bold text-xs" :class="step <= currentStep ? 'border-gray-800 bg-gray-100' : 'border-gray-300'">{{ step }}</div>
                ...
                <div v-if="step < totalSteps" class="flex-auto border-t-2 mx-3" :class="step < currentStep ? 'border-gray-800' : 'border-gray-300'"></div>
                </template>
            </div>
        </div>
        ...
        <div class="px-4 py-3 max-h-[55vh] overflow-y-auto space-y-3 border-t">
          ...
          <div v-show="currentStep === 1" class="space-y-3">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <FormInput label="Nama Pelanggan" v-model="form.payer" placeholder="Masukkan nama lengkap pelanggan" :error="errors.payer" :required="true" class="placeholder:text-xs"/>
              <FormInput label="Nomor Identitas" v-model="form.payer_identity" placeholder="Contoh: 3201234567890123 atau A1234567" :error="errors.payer_identity" :required="true" class="placeholder:text-xs"/>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              <FormSelect label="Jenis Kelamin" v-model="form.gender" :options="genders" placeholder="Pilih jenis kelamin" :error="errors.gender" :required="true" :class="!form.gender ? 'text-gray-500' : 'text-black'"/>
              <FormInput label="Tempat Lahir" v-model="form.birth_place" placeholder="Contoh: Jakarta, London, dll" :error="errors.birth_place" :required="true" class="placeholder:text-xs"/>
              <FormInput label="Tanggal Lahir" v-model="form.birth_date" type="date" :error="errors.birth_date" :required="true" class="placeholder:text-xs"/>
            </div>
            <FormInput label="Kewarganegaraan" v-model="form.nationality" placeholder="Contoh: Indonesia, Malaysia, Singapura, dll" :error="errors.nationality" :required="true" class="placeholder:text-xs"/>
          </div>

          <div v-show="currentStep === 2" class="space-y-3">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <FormSelect label="Jenis Permohonan Visa" v-model="form.jenis_visa" :options="visaTypeOptions" placeholder="Pilih jenis visa" :required="true" :error="errors.jenis_visa" :class="!form.jenis_visa ? 'text-gray-500' : 'text-black'"/>
              <FormInput label="Nomor Passport" v-model="form.passport_number" placeholder="Contoh: A1234567, X9876543" :error="errors.passport_number" :required="true" class="placeholder:text-xs"/>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
              <FormInput label="Tempat Dikeluarkan Passport" v-model="form.passport_issued_place" placeholder="Contoh: Jakarta, Kuala Lumpur" :required="true" :error="errors.passport_issued_place" class="placeholder:text-xs"/>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <FormInput label="Pekerjaan di Indonesia" v-model="form.indonesia_job" placeholder="Contoh: Karyawan, Wiraswasta, Mahasiswa" :required="true" :error="errors.indonesia_job" class="placeholder:text-xs"/>
              <FormInput label="Pekerjaan di Luar Negeri" v-model="form.abroad_job" placeholder="Contoh: Engineer, Teacher, Tourist" :required="true" :error="errors.abroad_job" class="placeholder:text-xs"/>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-800">Alamat Pekerjaan <span class="text-red-500">*</span></label>
              <textarea v-model="form.work_address" rows="2" placeholder="Masukkan alamat lengkap tempat kerja" class="w-full mt-1 px-3 py-2 border rounded-md text-black placeholder:text-gray-400 text-sm placeholder:text-xs" :class="errors.work_address ? 'border-red-500' : 'border-gray-300'"></textarea>
                 <p v-if="errors.work_address" class="text-xs text-red-600 mt-1">{{ errors.work_address }}</p>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <FormInput label="Kode Pos" v-model="form.postal_code" placeholder="Contoh: 12345, 10110" :required="true" :error="errors.postal_code" class="placeholder:text-xs"/>
              <FormSelect label="Kota Alamat" v-model="form.city" :options="cityOptions" placeholder="Pilih kota" :required="true" :error="errors.city" :class="!form.city ? 'text-gray-500' : 'text-black'"/>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <FormInput label="Negara Asal" v-model="form.origin_country" placeholder="Contoh: Indonesia, Malaysia, Thailand" :required="true" :error="errors.origin_country" class="placeholder:text-xs"/>
              <FormInput label="Nomor Telepon" v-model="form.phone" placeholder="Contoh: +62812345678, 08123456789" :error="errors.phone" :required="true" class="placeholder:text-xs"/>
            </div>
          </div>
          ...
          <div v-show="currentStep === 3" class="space-y-3">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                 ...
                 <div>
                    ...
                    <input type="text" :value="formatPrice(form.price)" @input="form.price = unformatPrice(($event.target as HTMLInputElement).value)" placeholder="Contoh: 1500000" class="w-full mt-1 px-3 py-2 border rounded-md font-semibold text-black text-sm placeholder:text-xs" :class="errors.price ? 'border-red-500' : 'border-gray-300'" />
        ...
        <div class="bg-gray-50 px-4 py-3 flex items-center justify-between rounded-b-lg">
          <div class="flex items-center gap-2">
            <button @click="resetFormAndClose" type="button" class="px-4 py-2 bg-white border border-gray-300 text-gray-800 rounded-lg hover:bg-gray-100 font-semibold transition-colors duration-200 text-sm">
              Cancel
            </button>
            <button v-if="currentStep > 1" @click="prevStep" type="button" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 font-semibold text-sm">
              Kembali
            </button>
          </div>
          <div class="flex items-center gap-2">
            <button v-if="currentStep === totalSteps" @click="saveCurrentEntry" :disabled="isLoading" class="px-3 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 font-semibold disabled:opacity-50 text-sm">
              Simpan Entry Ini
            </button>
            <PrimaryButton v-if="currentStep < totalSteps" @click="nextStep">
              Selanjutnya
            </PrimaryButton>
            <PrimaryButton v-if="currentStep === totalSteps" @click="saveAllEntries" :is-loading="isLoading">
