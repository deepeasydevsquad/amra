...
import { addKota, editKota } from '@/service/daftar_kota';
...
const validateForm = (): boolean => {
  ...
  if (!Kota.value.kode?.trim()) {
    errors.value.kode = 'Kode tidak boleh kosong';
    isValid = false;
  }
  if (!Kota.value.name?.trim()) {
...
const saveData = async () => {
  ...
  const isEdit = !!Kota.value.id;
  const action = async () => {
    try {
      if (isEdit) {
        const response = await editKota(Kota.value.id, Kota.value );
        showConfirmDialog.value = false;
        emit('status', { error: false, err_msg: (response?.error_msg || response?.message) || 'Kota berhasil di update' });
      } else {
        const response = await addKota(Kota.value);
        showConfirmDialog.value = false;
...
onMounted(() => {
  Kota.value = { ...Kota.value, ...(props.selectedKota ?? {})};
})
</script>
      ...
      <div class="relative inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <h3 class="text-2xl flex justify-center font-bold leading-6 text-gray-900 mb-4">
            {{ Kota.id ? "Edit Data Kota" : "Tambah Kota Baru" }}
          </h3>
          <div class="space-y-4">
            <div>
        ...
        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
          <PrimaryButton
            @click="saveData"
          >
            {{ Kota.id ? "Simpan Perubahan" : "Tambah" }}
          </PrimaryButton>
          <button
            @click="emit('close')"
