...
router.post("/trans_tiket/add_tiket", authenticateToken, 
    [
        body("tickets")
          .notEmpty().withMessage("Tickets tidak boleh kosong.")
          .custom((value) => {
            let parsed;
            try {
            ...
            parsed.forEach((ticket, index) => {
              ...
              if (!ticket.customer_price || isNaN(ticket.customer_price)) {
                throw new Error(`Ticket #${index + 1}: customer_price tidak valid.`);
              }
            });
        ...
        body("customer")
          ...
          .custom((value) => {
            ...
            if (!parsed.costumer_identity) {
              throw new Error("Field 'costumer_identity' pada customer harus diisi.");
            }
    
            return true;
          }),
    
        body("nomor_register").notEmpty().withMessage("Nomor register harus diisi."),
        body("invoice").optional(), // Invoice bisa kosong jika dibayar = 0
      ],
controllers.addTiket);

router.get("/trans_tiket/generate_nomor_register", authenticateToken, controllers.generateNomorRegister);
router.get("/trans_tiket/generate_nomor_invoice", authenticateToken, controllers.generateNomorInvoice);
router.get("/trans_tiket/ticket_transactions", authenticateToken,
      [
        query("pageNumber").trim().notEmpty().withMessage("Page Number tidak boleh kosong."),
        query("perpage").trim().notEmpty().withMessage("Jumlah Per Page tidak boleh kosong."),
        query("search").trim(),
      ],
