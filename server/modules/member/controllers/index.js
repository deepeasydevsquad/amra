...
exports.infoEditMember = async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(data).length > 0) {
      res.status(200).json({
        error: false,
        error_message: "Data berhasil ditemukan",
        data: data,
      });
    } else {
      res
        .status(400)
        .json({ error: true, error_message: "Data Gagal ditemukan" });
    }
  } catch (error) {
    handleServerError(res, error.message);
  }
...
exports.update = async (req, res) => {
  try {
    ...
    if (await model.response()) {
      res.status(200).json({
        error: false,
        error_msg: "Data Member Berhasil Diperbaharui.",
      });
    } else {
      res
        .status(400)
        .json({ error: true, error_msg: "Data Member Gagal Diperbaharui." });
    }
    // res.status(200).json(data);
  } catch (error) {
    handleServerError(res, error.message);
...
exports.delete = async (req, res) => {
  try {
    ...
    if (await model.response()) {
      res
        .status(200)
        .json({ error: false, error_msg: "Data Member Berhasil Dihapus." });
    } else {
      res
        .status(400)
        .json({ error: true, error_msg: "Data Member Gagal Dihapus." });
    }
  } catch (error) {
    handleServerError(res, error.message);
  }
...
exports.importExcel = async (req, res) => {
  ...
  try {
    const model = new Model_cud(req);
    await model.importExcel();
    // get response
    if (await model.response()) {
      res.status(200).json({
        error: false,
        error_msg: "Import Data Member Berhasil Dilakukan.",
      });
    } else {
      res.status(400).json({
        error: true,
        error_msg: "Import Data Member Gagal Dilakukan.",
      });
    }
  } catch (error) {
    handleServerError(res, error.message);
